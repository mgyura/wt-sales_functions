...
(function (a) {
    ...
    a.fn.timePicker = function (b) {
        ...
        q.append(r), q.appendTo("body").hide(), q.mouseover(
            function () {
                i = !0
            }).mouseout(function () {
            i = !1
...
jQuery(document).ready(function ($) {
    ...
    // File and itemimage upload handling
    //-------------------------------------------------------------------------------------------//
    var formfieldnew;
    var uploadStatus = true;

    $('.pydnet.').change(function () {
        formfieldnew = $(this).attr('name');
        $('#' + formfieldnew + '_id').val("");
    });

    $('.pydupload_button').live('click', function () {
        formfieldnew = $(this).prev('input').attr('name');
        buttonLabel = "Use as " + $('label[for=' + formfieldnew + ']').text();
    ...
    $('.remove_file_button').live('click', function () {
        formfieldnew = $(this).attr('rel');
        $('input#' + formfieldnew).val('');
        $('input#' + formfieldnew + '_id').val('');
        $(this).parent().remove();
        return false;
    });
    ...
    window.pydnetoriginal_send_to_editor = window.send_to_editor;
    window.send_to_editor = function (html) {
        if (formfieldnew) {
            ...
            var itemimage = /(^.*\.jpg|jpeg|png|gif|ico*)/gi;
            var document = /(^.*\.pdf|doc|docx|ppt|pptx|odt|psd|eps|ai*)/gi;
            var audio = /(^.*\.mp3|m4a|ogg|wav*)/gi;
            var video = /(^.*\.mp4|m4v|mov|wmv|avi|mpg|ogv|3gp|3g2*)/gi;

            if (itemurl.match(itemimage)) {
                uploadStatus = '<div class="img_status"><img src="' + itemurl + '" alt="" /><a href="#" class="remove_file_button" rel="' + formfieldnew + '">Remove Image</a></div>';
            } else {
                // No output preview if it's not an image
                // Standard generic output if it's not an image.
                html = '<a href="' + itemurl + '" target="_blank" rel="external">View File</a>';
                uploadStatus = '<div class="no_image"><span class="file_link">' + html + '</span>&nbsp;&nbsp;&nbsp;<a href="#" class="remove_file_button" rel="' + formfieldnew + '">Remove</a></div>';
            }

            $('#' + formfieldnew).val(itemurl);
            $('#' + formfieldnew + '_id').val(itemid);
            $('#' + formfieldnew).siblings('.cmb_upload_status').slideDown().html(uploadStatus);
            tb_remove();

        } else {
            window.pydnetoriginal_send_to_editor(html);
        }
        // Clear the formfieldnew value so the other media library popups can work as they are meant to. - 2010-11-11.
        formfieldnew = '';
    }
});